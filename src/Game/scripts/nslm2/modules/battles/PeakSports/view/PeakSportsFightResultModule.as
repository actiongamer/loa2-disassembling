...
{
   import game.ui.peakSports.peakSportsFightResult.PeakSportsResultModuleUI;
   import nslm2.modules.battles.PeakSports.model.vos.FastPeakData;
   import nslm2.modules.battles.PeakSports.control.PeakSportsFightResultRenderControl;
   ...
   public class PeakSportsFightResultModule extends PeakSportsResultModuleUI
   {
       
      
      private var _notify:FastPeakData;
      
      private var _leftRenderCtrl:PeakSportsFightResultRenderControl;
      
      private var _rightRenderCtrl:PeakSportsFightResultRenderControl;
      ...
      public function PeakSportsFightResultModule()
      {
         super();
      ...
      {
         _leftRenderCtrl = new PeakSportsFightResultRenderControl().bind(this.render_left);
         this.parts.push(new PeakSportsFightResultRenderControl().bind(this.render_left));
         _leftRenderCtrl.isLeft = true;
         _rightRenderCtrl = new PeakSportsFightResultRenderControl().bind(this.render_right);
         this.parts.push(new PeakSportsFightResultRenderControl().bind(this.render_right));
         _notify = param1 as FastPeakData;
         ...
         this.img_bg.skin = "modules/battlefields/ui/img_moduleBg.png";
         PeakSportsMsgs.ins.dispatchEvent(new MzEvent("msgBattleStart"));
         this.preShowCpl();
      }
      ...
      private function showLoss() : void
      {
         ...
         {
            if(PeakSportsModel.ins.isInBattleScene)
            {
               PeakSportsMsgs.ins.dispatchEvent(new MzEvent("msgFightPlayerDied",new PeakSportsPlayerDiedVo(PlayerModel.ins.playerInfo.id,_openStamp,PeakSportsModel.ins.currentBkStagePlayerInfo.conDeathNum,PeakSportsModel.ins.currentBkStagePlayerInfo.rebornStamp)));
               _loc1_ = new BkSceneMoveNotify();
               _loc1_.x = PeakSportsModel.ins.currentBkStagePlayerInfo.x;
               _loc1_.y = PeakSportsModel.ins.currentBkStagePlayerInfo.y;
               _loc1_.legal = false;
               _loc1_.id = PlayerModel.ins.playerInfo.id;
               PeakSportsMsgs.ins.dispatchEvent(new MzEvent("msgFightPlayerPosChanged",_loc1_));
            }
         }
         winOrLostCallBackExecuted = true;
         showLeaveBtn();
      ...
      override public function dispose() : void
      {
         ...
         _notify = null;
         PeakSportsMsgs.ins.dispatchEvent(new MzEvent("msgBattleEnd"));
         super.dispose();
      }
   }
}

package nochump.util.zip
{
   ...
   public class ZipFile
   {
      ...
      {
         var _loc1_:uint = buf.length - ZipConstants.ENDHDR;
      ...
      private function readEntries() : void
      {
         ...
         while(_loc1_ < entryList.length)
         {
            ...
            buf.position = buf.position + _loc2_.readUnsignedShort();
      ...
      public function getInput(param1:ZipEntry) : ByteArray
      {
         ...
         var _loc2_:uint = buf.readShort();
         buf.position = buf.position + (param1.name.length + _loc2_);

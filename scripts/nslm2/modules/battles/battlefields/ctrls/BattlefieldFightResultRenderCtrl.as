package nslm2.modules.battles.battlefields.ctrls
{
   ...
   public class BattlefieldFightResultRenderCtrl implements IDispose
   {
      ...
      public var bgFightInfo:BgFightInfo;
      ...
      public var playerData:FastBattlePlayerData;
      
      public function BattlefieldFightResultRenderCtrl()
      {
         _initPlayers = [];
         _resultPlayers = [];
         super();
      ...
      public function bind(param1:View) : BattlefieldFightResultRenderCtrl
      {
         render = param1;
         return this;
      }
      
      public function setData(param1:FastBattlePlayerData, param2:BgFightInfo) : void
      {
         bgFightInfo = param2;
         playerData = param1;
         validateAll();
      ...
      public function validateBuffIcon() : void
      {
         ...
         if(ref_buff_icon)
         {
            ...
            if(playerData.noviceBuffNum > 0)
            {
               _loc4_ = (DefindConsts.BG_NOVICE_BUFF_ID as String).split("|")[0];
               _loc5_ = StcMgr.ins.getBuffVo(_loc4_);
               _loc1_ = playerData.noviceBuffNum;
               _loc2_ = new BuffRenderS1UI();
               _loc2_.dataSource = new BuffHasLayersVo(_loc4_,_loc1_);
               this.ref_buff_icon.addChild(_loc2_);
               _loc2_.toolTip = LocaleMgr.ins.getStr(50500186,[_loc1_,4 - _loc1_,int(_loc1_ * _loc5_.init_effect_value1 * 100)]);
               _loc2_.x = _loc3_;
               if(isLeft)
               {
                  _loc3_ = _loc3_ + (_loc2_.width + 6);
      ...
      public function validateHeroes(param1:Boolean) : void
      {
         if(ref_list_heroes)
         {
            ...
            {
               initPlayers();
               this.ref_list_heroes.dataSource = BattlefieldFightHeroVo.createFightHeroArr(_initPlayers,param1);
            }
            else
            {
               this.ref_list_heroes.dataSource = BattlefieldFightHeroVo.createFightHeroArr(_resultPlayers,param1);
            }
         }
      }
